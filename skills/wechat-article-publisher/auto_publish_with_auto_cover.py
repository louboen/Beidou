...
COVER_HTML_TEMPLATE = os.path.join(ASSETS_DIR, "cover_template.html")
...
TOPIC_LIBRARY = {
    "节气养生": [
        {"title": "谷雨养生：雨生百谷，祛湿健脾", "subtitle": "雨生百谷 祛湿健脾", "color": "#2d6b45"},
        {"title": "清明养生：踏青祭祖，不忘养肝护阳", "subtitle": "踏青祭祖 养肝护阳", "color": "#4a8f5a"},
    ],
    "中医食疗": [
        {"title": "失眠怎么办？试试这 3 款安神汤", "subtitle": "食疗安神 改善睡眠", "color": "#5b6b9f"},
        {"title": "脾胃虚弱？喝这碗粥调理", "subtitle": "健脾养胃 易消化", "color": "#c9a959"},
    ],
    "穴位保健": [
        {"title": "内关穴：护心穴，缓解心悸胸闷", "subtitle": "宁心安神 缓解心悸", "color": "#b85c5c"},
        {"title": "足三里：长寿穴，健脾养胃", "subtitle": "强身健体 延年益寿", "color": "#6b8f5b"},
    ]
}
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .cover {{
            width: 900px;
            height: 500px;
            background: linear-gradient(135deg, {color} 0%, {adjust_color(color, 30)} 50%, {adjust_color(color, 50)} 100%);
            position: relative;
            overflow: hidden;
    ...
    <section style="margin-top: 40px; padding: 20px; background-color: #f5f5f5; border-radius: 10px;">
        ...
        <section style="display: flex; justify-content: center; align-items: flex-start; margin-bottom: 20px;">
            <section style="flex: 0 0 150px; margin-right: 20px;">
                <img src="{doctor_img_url}" style="width: 150px; height: 200px; object-fit: cover; border-radius: 10px;"/>
            </section>
...
def main():
    ...
    # 2. 生成封面图片
    print("\n🎨 生成封面图片...")
    cover_path = generate_cover_image(topic_info['title'], topic_info['subtitle'], topic_info['color'])
