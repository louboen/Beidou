...
<div style="width:900px;height:500px;background:linear-gradient(135deg,{color} 0%,{adjust_color(color,30)} 50%,{adjust_color(color,50)} 100%);position:relative;overflow:hidden;">
...
def generate_cover_with_browser(title, subtitle, color):
    ...
    # 启动 HTTP 服务器
    server_proc = subprocess.Popen(['python3', '-m', 'http.server', '8892'], cwd=ASSETS_DIR, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(2)
    ...
    try:
        ...
        open_result = subprocess.run(
            ['openclaw', 'browser', 'open', '--url', f'http://localhost:8892/cover_{timestamp}.html', '--target', 'host'],
