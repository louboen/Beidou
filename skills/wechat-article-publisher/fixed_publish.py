...
def create_cover_image(title, subtitle, color):
    ...
    # 启动 HTTP 服务器
    server_proc = subprocess.Popen(['python3', '-m', 'http.server', '8900'], cwd=ASSETS_DIR, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(2)
    
    try:
        # 打开页面
        print("📖 打开页面...")
        cmd1 = "openclaw browser open --url 'http://localhost:8900/%s' --target host" % html_filename
...
def main():
    ...
    if cover_path:
        ...
        if media_id:
            ...
            print("\n✅ 封面已上传到素材库")
            print("   MediaID: %s" % cover_media_id)
        else:
            print("\n⚠️  上传失败，使用医生照片")
    else:
        print("\n⚠️  生成失败，使用医生照片")
